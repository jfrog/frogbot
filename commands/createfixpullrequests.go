...
func (cfp *CreateFixPullRequestsCmd) openFixingPullRequest(impactedPackage, fixBranchName string, fixVersionInfo *utils.FixVersionInfo) (err error) {
	...
	commitString := cfp.gitManager.GenerateCommitMessage(impactedPackage, fixVersionInfo.FixVersion)
...
func (cfp *CreateFixPullRequestsCmd) addVulnerabilityToFixVersionsMap(vulnerability *formats.VulnerabilityOrViolationRow, fixVersionsMap map[string]*utils.FixVersionInfo) error {
	...
	vulnFixVersion, err := getFixVersion(vulnerability.ImpactedDependencyVersion, vulnerability.FixedVersions)
	if err != nil {
		return err
	}
	if vulnFixVersion == "" {
...
func getFixVersion(impactedPackageVersion string, fixVersions []string) (string, error) {
	...
	if idx == len(fixVersions) {
		return fixVersions[len(fixVersions)-1], nil
	}
	if idx == 0 {
		return fixVersions[0], nil
	}
	...
	v, err := utils.GetFixVersionSuggestion(version.NewVersion(currVersion), version.NewVersion(lower), version.NewVersion(upper))
	if err != nil {
		return "", err
	}
	if v != nil {
