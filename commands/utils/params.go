...
func NewConfigAggregatorFromFile(configFileContent []byte, gitParams *Git, server *coreconfig.ServerDetails) (result FrogbotConfigAggregator, err error) {
	...
	for _, repository := range result {
		gitParams.RepoName = repository.RepoName
		if repository.Branches != nil {
			gitParams.Branches = repository.Branches
		}
		repository.Git = *gitParams
		repository.Server = *server
		repository.OutputWriter = GetCompatibleOutputWriter(repository.GitProvider)
	}
	return
}
