...
func TestFormattedApplicabilityText(t *testing.T) {
	tests := []struct {
		...
		{vcsutils.GitHub, "undetermined", "$\\color{orange}{\\textsf{undetermined}}$"},
		...
		{vcsutils.GitLab, "undetermined", "$\\color{orange}{\\textsf{undetermined}}$"},
		...
		{vcsutils.AzureRepos, "undetermined", "<span style=\"color: orange;\">undetermined</span>"},
