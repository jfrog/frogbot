...
func TestFormattedApplicabilityText(t *testing.T) {
	tests := []struct {
		...
		{vcsutils.GitHub, "undetermined", "$\\color{#FFFFFFFF}{\\textsf{undetermined}}$"},
		...
		{vcsutils.GitLab, "undetermined", "$\\color{#FFFFFFFF}{\\textsf{undetermined}}$"},
		...
		{vcsutils.AzureRepos, "undetermined", "<span style=\"color: #FFFFFFFF;\">undetermined</span>"},
