...
func TestExtractScanPullRequestParamsFromEnv(t *testing.T) {
	...
	SetEnvAndAssert(t, map[string]string{IncludeAllVulnerabilitiesEnv: "TRUE", FailOnSecurityIssuesEnv: "FALSE", UseGradleWrapperEnv: "FALSE"})
	err = extractScanPullRequestParamsFromEnv(params)
	assert.NoError(t, err)
	...
	SetEnvAndAssert(t, map[string]string{IncludeAllVulnerabilitiesEnv: "true", FailOnSecurityIssuesEnv: "false", UseGradleWrapperEnv: "no"})
	err = extractScanPullRequestParamsFromEnv(params)
