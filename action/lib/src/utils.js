...
class Utils {
    static addToPath() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // Try Actions Cache first (persists across GitHub-hosted runners)
            if (yield this.loadFromActionsCache(version, fileName)) {
                return;
            }
            // Fallback to tool-cache (works for self-hosted runners)
            ...
            // Cache 'frogbot' executable in both caches
            yield this.saveToActionsCache(version, fileName, downloadDir);
    ...
    /**
     * Try to load Frogbot executable from Actions Cache.
     * @param version  - Frogbot version
     * @param fileName - 'frogbot' or 'frogbot.exe'
     * @returns true if the CLI executable was loaded from Actions Cache and added to path
     */
    static loadFromActionsCache(version, fileName) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const cacheKey = `frogbot-${(0, os_1.platform)()}-${version}`;
                const cachePath = (0, path_1.join)((0, os_1.homedir)(), '.cache', 'frogbot');
                const cachePaths = [cachePath];
                core.debug(`Attempting to restore Frogbot from Actions Cache with key: ${cacheKey}`);
                const restoredKey = yield cache.restoreCache(cachePaths, cacheKey);
                if (restoredKey) {
                    core.info(`Frogbot CLI restored from Actions Cache: ${restoredKey}`);
                    const execPath = (0, path_1.join)(cachePath, fileName);
                    // Make executable on Unix systems
                    if (!Utils.isWindows()) {
                        (0, fs_1.chmodSync)(execPath, 0o555);
                    }
                ...
                core.debug(`Actions Cache restore failed: ${error}`);
            }
            return false;
        });
    }
    /**
     * Save Frogbot executable to Actions Cache.
     * @param version     - Frogbot version
     * @param fileName    - 'frogbot' or 'frogbot.exe'
     * @param downloadDir - The directory whereby the CLI was downloaded to
     */
    static saveToActionsCache(version, fileName, downloadDir) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const cacheKey = `frogbot-${(0, os_1.platform)()}-${version}`;
                const cachePath = (0, path_1.join)((0, os_1.homedir)(), '.cache', 'frogbot');
                const cachePaths = [cachePath];
                // Ensure cache directory exists and copy binary
                (0, fs_1.mkdirSync)(cachePath, { recursive: true });
                (0, fs_1.copyFileSync)(downloadDir, (0, path_1.join)(cachePath, fileName));
                core.debug(`Attempting to save Frogbot to Actions Cache with key: ${cacheKey}`);
                yield cache.saveCache(cachePaths, cacheKey);
                core.info(`Frogbot CLI saved to Actions Cache: ${cacheKey}`);
            }
            catch (error) {
                core.debug(`Actions Cache save failed: ${error}`);
            }
        });
    }
