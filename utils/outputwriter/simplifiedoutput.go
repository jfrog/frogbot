...
`,
		...
		getSourceCodeTableContent(iacRows, smo))
}

func (smo *SimplifiedOutput) SastContent(sastRows []formats.SourceCodeRow) string {
	if len(sastRows) == 0 {
		return ""
	}
...
`,
		sourceCodeTableHeader,
		getSourceCodeTableContent(sastRows, smo))
}
