...
When installing Frogbot using JFrog Pipelines, Jenkins and Azure DevOps, Frogbot will not wait for a maintainer approval before scanning newly opened pull requests. Using Frogbot with these platforms however isn't recommended for open source projects.
When installing Frogbot using GitHub Actions and GitLab however, Frogbot will initiate the scan only after it is approved by a maintainer of the project, and therefore it is safe to be used for open source projects.
